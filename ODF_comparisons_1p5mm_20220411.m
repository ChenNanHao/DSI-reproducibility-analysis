...
people = People{aa};
    disp(append('Start working for ',People{aa}, '_1.5mm at ', datestr(datetime(now,'ConvertFrom','datenum'))))
    if aa > 11
...
% Pre
    cd(append('E:\dsi_data_7T_20200901_try\SRC_Batch\',people(13:end),'_pre_1p5mm'))
...
% Post
    cd(append('E:\dsi_data_7T_20200901_try\SRC_Batch\',people(13:end),'_post_1p5mm'))
    load(dir('*17.mat').name)
    for cc = 1:bb
...
people = People{aa};
save(append('ODF_info_15_',people(13:end)),'Fa0','Fa1','Fa2','Fa3','Fa4','singlefibermask','crossfibermask','facondition','index','dimension','odf_faces','odf_vertices');
save(append('ODF_info_15_',people(13:end),'p'),'Fa0p','Fa1p','Fa2p','Fa3p','Fa4p','indexp','dimension');
save(append('ODF_repro_15_',people(13:end)),'Pa1','Pa4','Px4_1','Px4_4','ODFsim');

disp(append('Finished working for ',People{aa}, '_1.5mm at ', datestr(datetime(now,'ConvertFrom','datenum'))))
clear all
end

imshow(imrotate(Pa1(:,:,35),-90),[-2 10]);colormap jet;colorbar
imshow(imrotate(Pa4(:,:,35),-90),[-2 30]);colormap jet;colorbar

imshow(imrotate(ODFsim(:,:,35),-90),[]);colormap jet
imshow(imrotate(ODFsim(:,:,35).*WMmask(:,:,35),-90),[]);colormap jet
...
people = People{aa};
load(append('ODF_info_15_',people(13:end)),'singlefibermask','crossfibermask');
load(append('ODF_repro_15_',people(13:end)));
